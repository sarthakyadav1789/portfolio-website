class MouseParallax {
    ...
    init() {
        document.querySelectorAll(".counter-motion").forEach(el => {
            this.addElement(
                el,
                parseFloat(el.dataset.depth) || 0.2,
                parseFloat(el.dataset.move) || 1,
                parseFloat(el.dataset.rotate) || 1
            );
        });
    ...
    addElement(el, depth, moveDir = 1, rotateDir = 1) {
        this.items.push({
            el,
            depth,
            moveDir,
